...
func (p *Pwm) SetFrequency(freq int) {
    if freq <= 0 || freq > pwmSamplingRate{
        prevFreq := freq
        freq = clip(freq, 0, pwmSamplingRate)
        errors.NewWarning(errors.FREQUENCY, prevFreq, freq)
    }
...
func (p *Pwm) Write(dutyCycle int) error {
    if dutyCycle < 0 || dutyCycle > 100 {
        ...
        dutyCycle = clip(dutyCycle, 0, 100)
        errors.NewWarning(errors.DUTY_CYCLE, prevDutyCycle, dutyCycle)
    }
