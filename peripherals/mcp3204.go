...
// the MCP3204 is an analog-to-digital converter (ADC) that communicates via the
// SPI interface. it has a resolution of 12 bits
type Mcp3204 struct {
	spi  *comms.Spi
	vRef float64
}
...
func (m *Mcp3204) read(channel int) (int, error) {
	cmd := make([]byte, 3)
...
func (m *Mcp3204) Raw(channel int) (int, error) {
